...
public class SubscriptionPlanController {
    ...
    @Operation(
            summary = "Đăng ký gói mới (free kích hoạt ngay / trả phí trả về link thanh toán)",
            description = """
                    ### Request body
                    - `planId` (bắt buộc): ID gói muốn đăng ký.
                    - `autoRenew` (optional): true/false, mặc định false. Chỉ áp dụng khi gói **free**
                      (gói trả phí hiện tại chưa hỗ trợ auto-renew tự động).
                    - `paymentMethod` (bắt buộc **nếu gói trả phí**): `"VNPAY"` hoặc `"MOMO"`.
                      Bỏ qua field này nếu đăng ký gói free.

                    ### Response — 2 kịch bản

                    **Gói FREE** → `requiresPayment: false`
                    ```json
                    {
                      "success": true,
                    ...
                    **Gói TRẢ PHÍ** → `requiresPayment: true`
                    ```json
                    {
                      "success": true,
                    ...
                    ```
                    → **FE phải redirect user đến `paymentUrl` ngay lập tức** (VD: `window.location.href = paymentUrl`).

                    ### Các lỗi thường gặp
                    - `SUB_PLAN_001`: Gói không tồn tại.
                    - `SUB_PLAN_003`: Gói chưa được kích hoạt (inactive).
                    - `SUB_003`: Request không hợp lệ — ví dụ đang dùng gói khác mà cố "gia hạn" gói free khác loại,
                      hoặc thiếu `paymentMethod` khi đăng ký gói trả phí.
                    """
    )
    @SecurityRequirement(name = "bearerAuth")
